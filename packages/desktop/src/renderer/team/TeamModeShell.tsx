...
export function TeamModeShell({ embedded = false, teamView = "planning", onConnectionRoleChange, onExit }: { embedded?: boolean; teamView?: string; onConnectionRoleChange?: (role: "server" | "member") => void; onExit: () => void }) {
  ...
  async function hostTeam() {
    ...
    try { const host = await remoteBridge.startLocalTeamHost({ projectId, projectName, userId, deviceId, joinToken, allowInsecureLan }); setProfiles((current) => [...current.filter((profile) => profile.id !== host.profile.id), host.profile]); setInviteUrl(host.inviteUrl); onConnectionRoleChange?.("server"); await open(host.profile); }
    catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)); }
