...
export type ServerConfig = { dataDirectory: string; databasePath: string; host: string; port: number; busyTimeoutMs: number };
...
export function readServerConfig(env: NodeJS.ProcessEnv = process.env): ServerConfig {
  ...
  return { dataDirectory, databasePath: resolve(dataDirectory, "planweave-server.sqlite"), host: env.PLANWEAVE_SERVER_HOST?.trim() || "127.0.0.1", port, busyTimeoutMs };
}
